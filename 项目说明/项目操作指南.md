...
> ① **RAG 混合检索**——Milvus 语义向量 + PostgreSQL BM25 关键词双路召回，RRF 融合后按故障原因加权重排，用大白话描述故障即可命中历史案例（详见《检索全链路详解.md》）；
...
├── docker-compose.dev.yml    # 开发辅助编排（按需使用）
...
> 提示：
> - 首次同步向量会**自动下载 Embedding 模型（Qwen3-Embedding-0.6B，约 1GB）**，耗时较长属正常
...
**`start_all.ps1`**：自动完成"端口体检（冲突时弹 UAC 自动调用 fix_winnat_ports.ps1 修复）→ `docker compose up -d` → 后台启动后端(18080) + 前端(4173)"。
...
日志文件（项目根目录）：`backend.log` / `backend.err.log`（后端）、`frontend.log` / `frontend.err.log`（前端）。后端首次启动需加载 Embedding 模型，**约 1 分钟就绪**，期间接口不可用属正常。
...
| 检索不到新知识 | 新增知识后执行 `python scripts\sync_vectors.py` 同步向量 |
| 问答提示"模型加载中/接口不可用" | 后端首次启动需加载 Embedding 模型约 1 分钟，稍等再试 |
